...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  // Step 1: Handle posts_rels first (drop and recreate without FK to posts)
  ...
  // Step 2: Handle _posts_v (drop it temporarily since it references posts)
  await db.run(sql`CREATE TABLE \`__temp__posts_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_title\` text,
  	\`version_slug\` text,
  ...
  // Step 4: Recreate posts_rels with FK to posts
  ...
  // Step 5: Recreate _posts_v with FK to posts
  ...
  `)
  await db.run(
    sql`INSERT INTO \`_posts_v\`("id", "parent_id", "version_title", "version_slug", "version_studio_tag", "version_preview_description", "version_featured", "version_featured_image_id", "version_published_status", "version_published_at", "version_updated_at", "version_created_at", "version__status", "created_at", "updated_at", "latest") SELECT "id", "parent_id", "version_title", "version_slug", "version_studio_tag", "version_preview_description", "version_featured", "version_featured_image_id", "version_published_status", "version_published_at", "version_updated_at", "version_created_at", "version__status", "created_at", "updated_at", "latest" FROM \`__temp__posts_v\`;`,
  )
  await db.run(sql`DROP TABLE \`__temp__posts_v\`;`)

  // Step 6: Turn foreign keys back on
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_posts_rels\`("id", "order", "parent_id", "path", "users_id") SELECT "id", "order", "parent_id", "path", "users_id" FROM \`posts_rels\`;`,
  )
  await db.run(sql`DROP TABLE \`posts_rels\`;`)
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new__posts_v_rels\`("id", "order", "parent_id", "path", "users_id") SELECT "id", "order", "parent_id", "path", "users_id" FROM \`_posts_v_rels\`;`,
  )
  ...
  await db.run(
    sql`ALTER TABLE \`_posts_v\` ADD \`version_category_id\` integer REFERENCES categories(id);`,
  )
  await db.run(
